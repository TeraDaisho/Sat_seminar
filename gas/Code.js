...
const getLectures = () => {
    try {
        ...
        let sheet = ss.getSheetByName('フォームの回答 2');
        if (!sheet) {
            sheet = ss.getSheets()[0]; // Fallback to the first sheet if name doesn't match
        }
        if (!sheet) return [];
        ...
        const getColIndex = (keys) => headers.findIndex(h => keys.some(k => String(h).includes(k)));
        ...
        const idxTarget = getColIndex(['対象', 'target', 'こんな人におすすめ']);
